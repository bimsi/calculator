...
class Calculator:
    def __init__(self,master):
        ...
        self.entry_value=''
        Entry(width=17, bg='grey', font=('Arial Bold',30), textvariable=self.equation).place(x=0,y=0)

        Button(width=11, height=4, text='(', relief='flat', bg='white', command=lambda:self.show('(')).place(x=0, y=50)
        Button(width=11, height=4, text=')', relief='flat', bg='white', command=lambda:self.show(')')).place(x=90, y=50)
        Button(width=11, height=4, text='%', relief='flat', bg='black', command=lambda:self.show('%')).place(x=180, y=50)
